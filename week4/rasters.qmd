...
# find out the names of our layers
names(landsat)
```
...
```{r}
map3 <- tm_shape(austria) +
  tm_raster(col.legend = tm_legend("Elevation (m)")) +
  tm_title(text = "Austria")

map4 <- tm_shape(switzerland) +
  tm_raster(col.legend = tm_legend("Elevtation (m)")) +
  tm_title(text = "Switzerland")

map5 <- tm_shape(merged) +
  tm_raster(col.legend = tm_legend("Elevation(m)")) +
  tm_title(text = "Merged")

tmap_arrange(map4, map3, map5, nrow = 1)
```
